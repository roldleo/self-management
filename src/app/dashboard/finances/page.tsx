...
export default function TransactionPage() {
    ...
    const [type, setType] = useState<'pendapatan' | 'pengeluaran'>('pendapatan')
            ...
            <div className="md:col-span-2 space-y-4">
                ...
                <div className="grid gap-4 sm:grid-cols-2 md:grid-cols-3">
                    {transaction.map((item) => (
                        <div key={item.id} className={`rounded-2xl p-4 shadow-sm flex flex-col gap-3 text-white ${item.type === 'pendapatan' ? 'bg-green-300' : item.category === 'konsumtif' ? 'bg-blue-300' : item.category === 'pengeluaran wajib' ? 'bg-purple-300' : item.category === 'hiburan' ? 'bg-orange-300' : 'bg-gray-200'}`}>
                            <div className="flex flex-col">
                                <div className="font-semibold text-lg">{item.title}</div>
                                <div className="text-sm">
                                    {item.type} | Rp{item.amount.toLocaleString('id-ID')}
                                </div>
                                {item.type === 'pengeluaran' && <div className="text-xs">Kategori: {item.category}</div>}
                            </div>
